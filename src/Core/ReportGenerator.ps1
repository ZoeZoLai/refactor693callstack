                ...
                <div class="section-content">
                    ...
                    <div class="status-filter-container">
                        ...
                        <div class="filter-options">
                            <label class="filter-option">
                                <input type="radio" name="statusFilter" value="issues" checked>
                                <span class="filter-text">Issues Only (Fail & Warning)</span>
                            </label>
                            <label class="filter-option">
                                <input type="radio" name="statusFilter" value="fail">
                                <span class="filter-text">Fail Only</span>
                            </label>
                            <label class="filter-option">
                                <input type="radio" name="statusFilter" value="warning">
                                <span class="filter-text">Warning Only</span>
                            </label>
                            <label class="filter-option">
                                <input type="radio" name="statusFilter" value="pass">
                                <span class="filter-text">Pass Only</span>
                            </label>
                            <label class="filter-option">
                                <input type="radio" name="statusFilter" value="info">
                                <span class="filter-text">Info Only</span>
                            </label>
                            <label class="filter-option">
                                <input type="radio" name="statusFilter" value="all">
                    ...
                    <div class="health-results">
                        $(foreach ($result in $Results) {
                            ...
                            @"
                        <div class="health-result $statusClass $messageClass" data-status="$($result.Status.ToLower())">
...
"@
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var healthResults = document.querySelectorAll('.health-result');
            ...
            function updateResultCount(filterValue) {
                var visibleResults = document.querySelectorAll('.health-result:not(.filtered-hidden)');
                var totalResults = healthResults.length;
                
                // Create or update result count display
                var countDisplay = document.querySelector('.result-count-display');
                if (!countDisplay) {
                    countDisplay = document.createElement('div');
                    countDisplay.className = 'result-count-display';
                    countDisplay.style.cssText = 'margin-top: 10px; font-style: italic; color: #6c757d; font-size: 0.9em;';
                    
                    var healthResultsContainer = document.querySelector('.health-results');
                    var parentContainer = healthResultsContainer.parentNode;
                    parentContainer.insertBefore(countDisplay, healthResultsContainer);
                }
                
                var filterText = getFilterDisplayText(filterValue);
                countDisplay.textContent = 'Showing ' + visibleResults.length + ' of ' + totalResults + ' results (' + filterText + ')';
            }
            
            function getFilterDisplayText(filterValue) {
                switch(filterValue) {
                    case 'all': return 'All statuses';
                    case 'issues': return 'Fail & Warning only';
                    case 'fail': return 'Fail only';
                    case 'warning': return 'Warning only';
                    case 'pass': return 'Pass only';
            ...
            // Apply default filter on page load (Issues Only - Fail & Warning)
            filterResults('issues');
        });
    </script>
</body>
</html>
